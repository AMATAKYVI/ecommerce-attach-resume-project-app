...
import { addItem, removeCompletely, removeItem } from '../store/cartSlices';
              ...
              <div key={item.id}>
                ...
                <div className="flex py-10 ">
                  ...
                  <div className="flex-1 flex flex-col justify-between ">
                    <div>
                      <h1 className="font-semibold cursor-pointer mb-2">
                        {item.name}
                      </h1>
                  ...
                  <div
                    ...
                    onClick={() => dispatch(removeCompletely(item.id))}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
        ...
        <div className="my-20 border py-8 px-8 flex flex-col gap-5">
          ...
          <div className="border-b py-2 flex justify-between">
            ...
            <p className="font-semibold">
              $
              {(
                state.reduce((prev, curr) => prev + curr.totalPrice, 0) * 0.02
              ).toFixed(2)}
            </p>
          </div>
          <div className="border-b py-2 flex justify-between">
            ...
            <p className="font-semibold">
              $
              {(
                state.reduce((prev, curr) => prev + curr.totalPrice, 0) * 0.06
              ).toFixed(2)}
            </p>
          </div>
          <div className=" py-2 flex justify-between">
            ...
            <p className="font-semibold text-2xl">
              $
              {(
                state.reduce((prev, curr) => prev + curr.totalPrice, 0) +
                state.reduce((prev, curr) => prev + curr.totalPrice, 0) * 0.02 +
                state.reduce((prev, curr) => prev + curr.totalPrice, 0) * 0.06
              ).toFixed(2)}
            </p>
